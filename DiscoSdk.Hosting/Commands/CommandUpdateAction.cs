...
internal class CommandUpdateAction(DiscordClient client) : ICommandUpdateAction
{
    ...
    public async Task RegisterAsync(CancellationToken ct = default)
    {
        try
        {
            await RegisterGlobalCommandsAsync(ct);
            await RegisterGuildCommandsAsync(ct);
        }
        catch (Exception ex)
        {
    ...
    /// </summary>
    private async Task RegisterGlobalCommandsAsync(CancellationToken ct)
    {
        ...
        if (!_deletePrevious)
        {
            ...
            var existingGlobal = await _applicationCommandClient.GetGlobalCommandsAsync(client.ApplicationId!, ct);
        ...
        var registered = await _applicationCommandClient.RegisterGlobalCommandsAsync(client.ApplicationId!, commandsToSend, ct);
    ...
    /// </summary>
    private async Task RegisterGuildCommandsAsync(CancellationToken ct)
    {
        ...
        foreach (var (guildId, commands) in _guildCommands)
        {
            ...
            if (!_deletePrevious)
            {
                ...
                var existingGuild = await _applicationCommandClient.GetGuildCommandsAsync(client.ApplicationId!, guildId, ct);
            ...
            var guildRegistered = await _applicationCommandClient.RegisterGuildCommandsAsync(client.ApplicationId!, guildId, commandsToSend, ct);
