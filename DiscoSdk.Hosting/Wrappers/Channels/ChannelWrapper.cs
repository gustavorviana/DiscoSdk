...
internal class ChannelWrapper(Channel channel, DiscordClient client) : IChannel
{
	...
	{
		return new RestAction(cancellationToken => _client.ChannelClient.DeleteAsync(_channel.Id, cancellationToken));
	}
    ...
    public static IChannel ToSpecificType(Channel channel, IGuild? guild, DiscordClient client)
    {
        ...
        var unionWrapper = new ChannelUnionWrapper(channel, guild!, client);

        return channel.Type switch
        {
            ChannelType.Dm or ChannelType.GroupDm => unionWrapper.AsDmChannel() ?? (IChannel)unionWrapper,
            ...
            ChannelType.GuildCategory => unionWrapper.AsCategory() ?? (IChannel)unionWrapper,
