...
internal class InviteWrapper(Invite invite, IGuildChannelBase channel, DiscordClient client) : IInvite
{
    ...
    /// <inheritdoc />
    public IUser? Inviter { get; } = invite.Inviter != null ? new UserWrapper(client, invite.Inviter) : null;

    /// <inheritdoc />
    public int? TargetType => _invite.TargetType;

    /// <inheritdoc />
    public IUser? TargetUser { get; } = invite.TargetUser != null ? new UserWrapper(client, invite.TargetUser) : null;
