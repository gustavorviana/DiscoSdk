...
public class ApplicationCommandClientTests
{
    ...
    {
        _http.SendAsync<List<ApplicationCommand>>(Arg.Any<DiscordRoute>(), Arg.Any<HttpMethod>(), Arg.Any<object?>(), Arg.Any<CancellationToken>())
            .Returns(new List<ApplicationCommand>());

        var commands = new List<ApplicationCommand> { new() { Name = "ping" } };
        await _client.RegisterGlobalCommandsAsync(_appId, commands);

        await _http.Received(1).SendAsync<List<ApplicationCommand>>(
    ...
    public async Task RegisterGuildCommandsAsync_PutsToApplicationGuildCommandsRouteAsync()
    {
        ...
        await _client.RegisterGuildCommandsAsync(_appId, _guildId, new List<ApplicationCommand>());
    ...
    public async Task GetGlobalCommandAsync_GetsSingleCommandAsync()
    {
        ...
        var expected = new ApplicationCommand { Id = commandId, Name = "ping" };
        _http.SendAsync<ApplicationCommand>(Arg.Any<DiscordRoute>(), Arg.Any<HttpMethod>(), Arg.Any<object?>(), Arg.Any<CancellationToken>())
            .Returns(expected);

        var result = await _client.GetGlobalCommandAsync(_appId, commandId);

        Assert.Same(expected, result);
        await _http.Received(1).SendAsync<ApplicationCommand>(
    ...
    {
        var cmd = new ApplicationCommand { Name = "echo", Description = "echo back" };
        _http.SendAsync<ApplicationCommand>(Arg.Any<DiscordRoute>(), Arg.Any<HttpMethod>(), Arg.Any<object?>(), Arg.Any<CancellationToken>())
            .Returns(cmd);
    ...
    public async Task EditGlobalCommandAsync_PatchesByIdAsync()
    {
        var commandId = new Snowflake(42);
        _http.SendAsync<ApplicationCommand>(Arg.Any<DiscordRoute>(), Arg.Any<HttpMethod>(), Arg.Any<object?>(), Arg.Any<CancellationToken>())
            .Returns(new ApplicationCommand { Name = "x" });
    ...
    {
        var cmd = new ApplicationCommand { Name = "guild-cmd" };
        _http.SendAsync<ApplicationCommand>(Arg.Any<DiscordRoute>(), Arg.Any<HttpMethod>(), Arg.Any<object?>(), Arg.Any<CancellationToken>())
            .Returns(cmd);
