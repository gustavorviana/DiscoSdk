...
public interface IGuildTextChannelBase : IGuildChannelBase, ITextBasedChannel
{
	...
	Task DeleteAllReactionsAsync(DiscordId messageId, CancellationToken cancellationToken = default);
	...
	/// <param name="cancellationToken">A cancellation token to cancel the operation.</param>
	/// <returns>A task that represents the asynchronous operation.</returns>
	Task RemoveReactionAsync(DiscordId messageId, Emoji emoji, CancellationToken cancellationToken = default);
	...
	ISendMessageRestAction SendStickers(IEnumerable<DiscordId> stickers);
}